...
Namespace Default_MVVM
    ...
    Public Class FilterPopupConverter
        ...
        Public Function Convert(ByVal value As Object, ByVal targetType As Type, ByVal parameter As Object, ByVal culture As Globalization.CultureInfo) As Object Implements IValueConverter.Convert
            ...
            If source IsNot Nothing AndAlso source.Count > 0 Then
                ...
                If grid.IsFilterEnabled AndAlso Not column.IsFiltered Then
                    ...
                    Dim evaluator As ExpressionEvaluator = New ExpressionEvaluator(descriptors, grid.FilterCriteria)
        ...
        Private Function GetUniqueValues(ByVal source As ObservableCollection(Of TestDataViewModel)) As ObservableCollection(Of TestDataViewModel)
            Dim uniqueValues As ObservableCollection(Of TestDataViewModel) = New ObservableCollection(Of TestDataViewModel)()
            For Each dataRow As TestDataViewModel In source
                If uniqueValues.Where(Function(val) val.Number.Equals(dataRow.Number)).Count() = 0 Then uniqueValues.Add(dataRow)
            Next
    ...
    Public Class CriteriaOperatorConverter
        ...
        Public Function ConvertBack(ByVal value As Object, ByVal targetTypes As Type(), ByVal parameter As Object, ByVal culture As Globalization.CultureInfo) As Object() Implements IMultiValueConverter.ConvertBack
            If TypeOf value Is Integer Then Return New Object() {New BinaryOperator("Number", CInt(value), BinaryOperatorType.Equal), False}
